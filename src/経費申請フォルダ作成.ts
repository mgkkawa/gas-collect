...
function newExform() {
  ...
  const year = dateString(date, 'yyyy年');
  const month = dateString(date, 'M月');
  const sheetname = year + month;
  const foldername = dateString(date, 'yyyy.MM');
  ...
  sheetnamelast.forEach((value, index) => {
    ...
    if (index == 0) {
      exs[index].getRange(1, 2, 1, 2).setValues([[year, month]]);
    }
  });
  ...
  names.forEach((name, index) => {
    ...
    sheets.forEach((sheet, x) => {
      if (x != 0) {
        sheet.hideSheet();
        if (x == 1) {
          sheet.getRange('J6:J45').insertCheckboxes();
        }
      }
      else {
