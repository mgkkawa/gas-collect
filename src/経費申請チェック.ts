...
function calculation(e) {
  ...
  if (e != '月末分') {
    ...
    var sheet_data = sheet.getDataRange().getValues().filter(values => new Date(values[0]).getTime() >= date.getTime() &&
