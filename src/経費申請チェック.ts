...
const calculation = (e) => {
  ...
  const sheetname = valueDate(date, 'yyyy.M.d');
  ...
  name_list.forEach(staff => {
    ...
    const folder = root.getFoldersByName(valueDate(date, 'yyyy.MM')).next();
    ...
    const ex_sheet1 = exform.getSheetByName(valueDate(date, 'yyyy年M月'));
    ...
    if (e != '月末分') {
      ex_sheet2.getRange(ex_sheet2.getLastRow() + 1, 1, 1, 2)
        .setValues([[sum, valueDate(new Date(), 'yyyy/M/d')]]);
    }
