...
const shiftObjectAddInfo = (date = new Date(), obj = originCheck_(date)) => {
  ...
  const as_data = origin.filter((values) => Object.prototype.toString.call(values[check[0]]) == '[object Date]' &&
  ...
  trim_days.forEach(dd => {
    for (let staff of staffs) {
      if (obj[dd][staff]['FLAG']) {
        const dd_map = as_data.filter((values, index) => index >= days.indexOf(dd) && index <= days.lastIndexOf(dd) && values.includes(staff));
...
function ddObject_(array, label, staff) {
  ...
  array.forEach((values, index) => {
    ...
    keys.forEach(key => {
      switch (key) {
        case '開始':
          dd_obj[obj_[key]] = dateString(values[label.indexOf(key)], 'H:mm');
          break;
        case '終了':
          dd_obj[obj_[key]] = dateString(values[label.indexOf(key)], 'H:mm');
          break;
        case '講師':
          ...
          if (flag_ && main) {
            ...
            dd_obj[obj_[key]] = 'サポート';
          }
          else {
            dd_obj[obj_[key]] = 'SB同行';
          }
          break;
...
const shiftObjectUpdate_ = (obj, date = new Date()) => {
  ...
  for (let [index, data] of db_data.entries()) {
    ...
    if (db_yyyy == yyyy && db_M == M) {
      check_row.push(`B${(index + 2)}`);
    }
  }
  if (check_row.length > 0) {
    try {
      database.getRangeList(check_row).uncheck();
    }
    catch {
      database.getRange(check_row[0]).uncheck();
    }
  }
