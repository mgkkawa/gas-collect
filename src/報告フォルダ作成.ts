const folderCreate_ = () => {
  ...
  try { root.getFoldersByName(dateString(set_date, 'yyyy.MM')).next() }
  catch { root.createFolder(dateString(set_date, 'yyyy.MM')) }
  const month_folder = root.getFoldersByName(dateString(set_date, 'yyyy.MM')).next()

  try { month_folder.getFoldersByName(dateString(set_date, 'MM/dd')).next() }
  catch { month_folder.createFolder(dateString(set_date, 'MM/dd')) }
