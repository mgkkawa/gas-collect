const folderCreate_ = () => {
  ...
  try { root.getFoldersByName(valueDate(set_date, 'yyyy.MM')).next(); }
  catch { root.createFolder(valueDate(set_date, 'yyyy.MM')); }
  const month_folder = root.getFoldersByName(valueDate(set_date, 'yyyy.MM')).next();

  try { month_folder.getFoldersByName(valueDate(set_date, 'MM/dd')).next(); }
  catch { month_folder.createFolder(valueDate(set_date, 'MM/dd')); }
  const day_folder = month_folder.getFoldersByName(valueDate(set_date, 'MM/dd')).next();

  const as_sheet = mainData_('as').getSheetByName(valueDate(set_date, 'yyyyMM'));
