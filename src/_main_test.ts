  ...
  for (let day of trim_days) {
    ...
    for (let [index, values] of array.entries()) {
      if (index >= days.indexOf(day) && index <= days.lastIndexOf(day)) {
        ...
        ven_['SUPPORT'] = values.filter((value, ind) =>
          ind > label.indexOf('変更後メンバー') && ind < label.indexOf('SPLIT1') && value != '')
        obj_[`index${index}`] = ven_
      } else if (index > days.lastIndexOf(day)) { break } else { continue }
    }
    if (String(day.match(/^[\d]./)) == to_month) { to_obj[dd] = obj_ }
    else { next_obj[dd] = obj_ }
  }
  obj[to_month] = to_obj
  if (to_month != next_month) { obj[next_month] = next_obj }
  return obj
}
