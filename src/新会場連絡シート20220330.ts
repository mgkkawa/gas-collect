...
function writeVenCall() {
  ...
  const vcsd = origin_vc_data.map(values => vckeys.map((key, index) => {
    if (index != 3) { return dateString(values[key]) } else { return dateString(values[key], 'H:mm') }
  ...
  const assdays = assd.map(values => values.filter((value, index) =>
    index == asslabel.indexOf('日程'))).flat().map(value => dateString(value))
...
function writeCasting() {
  ...
  if (!cas_obj) {
    try { Browser.msgBox('対象のキャスティング情報がありませんでした。') }
    finally {
  ...
  const main_table = new ShiftTable(to_shsheet)
  ...
  if (cas_obj.check()) {
    ...
    sub_table = new ShiftTable(sub_sh, date)
  }
  ...
  if (deletecheck == 'yes') { casting.getRange('U2:Z500').clearContent() }
}
...
const holdCheck_ = () => {
  ...
  const vcag_days = vcagd.map(values => values.filter((value, index) =>
    index == vcag_label.indexOf('日程'))).flat().map(value => dateString(value))
